...
const API_URL =
  process.env.API_URL || "https://cms.expressnepal.com.com/?graphql";
    ...
    const posts = json.data.posts.edges.map((edge) => {
      ...
      if (post.featuredImage?.node?.sourceUrl) {
        ...
        if (imageUrl.startsWith("/")) {
          imageUrl = `https://cms.expressnepal.com${imageUrl}`;
...
export async function fetchPostBySlug(slug: string, categorySlug?: string): Promise<Post | null> {
  ...
  const fixImage = (p: Post | null): Post | null => {
    if (p?.featuredImage?.node?.sourceUrl?.startsWith("/")) {
      p.featuredImage.node.sourceUrl = `https://cms.expressnepal.com${p.featuredImage.node.sourceUrl}`;
    }
    return p;
  };
    ...
    const normalize = (posts: Post[]) =>
      posts.map((post) => {
        if (post.featuredImage?.node?.sourceUrl?.startsWith("/")) {
          post.featuredImage.node.sourceUrl = `https://cms.expressnepal.com${post.featuredImage.node.sourceUrl}`;
        }
        return post;
      });
